      ...
      const dayButton = (
        ...
        >
          ...
          {hasLeave && !isHolidayDate && (
            <FileText className="absolute -top-1 -right-1 w-2.5 h-2.5 sm:w-3 sm:h-3 text-blue-500" />
          )}
        </button>
      );
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6 mt-3 sm:mt-4">
              ...
              {Object.keys(balanceUsage).length > 0 && (
                <div className="p-2 sm:p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <Label className="text-xs sm:text-sm font-medium text-blue-900 mb-1.5 sm:mb-2 block">
        ...
        <CardContent className="p-3 sm:p-4">
          <div className="flex items-start gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              <h3 className="font-semibold text-blue-900 mb-1.5 sm:mb-2 text-xs sm:text-sm">Leave Policy</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4 text-[10px] sm:text-sm text-blue-800">
                {leaveTypes.filter(t => !t.isCompOff && t.name !== 'Unpaid Leave').slice(0, 3).map(type => (
                  <div key={type.name}>
                    <p className="font-medium truncate flex items-center gap-1">
                      {type.name === 'Earned Leave' && <Calendar className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600 shrink-0" />}
                      {type.name === 'Sick Leave' && <Stethoscope className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600 shrink-0" />}
                      {type.name === 'Casual Leave' && <Target className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600 shrink-0" />}
                      <span>{type.name}</span>
                    </p>
